...
@section('content')
  <form action="/dashboard/plants" method="post" enctype="multipart/form-data">
    @csrf
    <div class="row mt-3">
      <div class="col-12 col-md-8 col-lg-6">
        <div class="card">
          <div class="card-body">
            <div class="card-title">Tambah Tanaman</div>
            <hr>
            <div class="form-group">
              <label for="name">Nama Tanaman</label>
              <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                placeholder="Masukkan nama tanaman" value="{{ old('name') }}" autofocus>
              @error('name')
            ...
            <div class="form-group">
              ...
              <select name="plant_category_id" id="kategori"
                ...
                @foreach ($plant_categories as $plant_category)
                  @if (old('plant_category_id') == $plant_category->id)
            ...
            <div class="form-group">
              ...
              <textarea class="form-control @error('detail') is-invalid @enderror" id="detail" name="detail"
                placeholder="Masukkan detail tanaman" rows="5">{{ old('detail') }}</textarea>
              @error('detail')
                <div class=" invalid-feedback">
                  {{ $message }}
                </div>
